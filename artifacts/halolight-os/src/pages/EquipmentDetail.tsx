...
const SERVICE_TYPE_CONFIG: Record<string, { label: string; color: string; dot: string }> = {
  routine_maintenance: { label: "Routine Maintenance", color: "bg-blue-50 text-blue-700 border-blue-200", dot: "bg-blue-400" },
  repair:              { label: "Repair",              color: "bg-amber-50 text-amber-700 border-amber-200", dot: "bg-amber-400" },
  upgrade:             { label: "Upgrade",             color: "bg-purple-50 text-purple-700 border-purple-200", dot: "bg-purple-400" },
  inspection:          { label: "Inspection",          color: "bg-green-50 text-green-700 border-green-200", dot: "bg-green-400" },
  warranty_claim:      { label: "Warranty Claim",      color: "bg-red-50 text-red-700 border-red-200", dot: "bg-red-400" },
};
...
function warrantyInfo(expiry: string | null) {
  if (!expiry) return { label: "No warranty on file", color: "text-gray-400", Icon: ShieldX, alert: false };
  const now = new Date();
  ...
  return { label: `Valid until ${fmtDate(expiry, { month: "long", year: "numeric" })}`, color: "text-green-600", Icon: ShieldCheck, alert: false };
}
...
export default function EquipmentDetail() {
  ...
  if (!item) return (
    <div className="max-w-3xl mx-auto text-center py-20 text-gray-400">Equipment not found</div>
  );
    ...
    <div className="max-w-3xl mx-auto space-y-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-xl font-bold text-gray-900">{item.productModel}</h1>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-4">
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-700">{fmtDate(item.purchaseDate)}</p>
                </div>
                {item.purchasePrice && (
                  <div>
                    <p className="text-xs text-gray-400 mb-0.5">Purchase Price</p>
                    <p className="text-sm font-medium text-gray-700">${Number(item.purchasePrice).toLocaleString()}</p>
                  </div>
                )}
                {item.vendorName && (
                  <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Card className={warranty.alert ? "border-amber-200 bg-amber-50/30" : ""}>
          <CardContent className="p-4">
            ...
            {item.warrantyExpiration && (
              <p className="text-xs text-gray-400 mt-0.5">Expiry: {fmtDate(item.warrantyExpiration)}</p>
            )}
          </CardContent>
        </Card>
        <Card className={maintenance.urgent ? "border-amber-200 bg-amber-50/30" : ""}>
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-2">
              <Wrench className={`w-4 h-4 ${maintenance.urgent ? "text-amber-500" : "text-gray-400"}`} />
              <p className="text-sm font-semibold text-gray-700">Maintenance</p>
            </div>
            <p className={`text-sm font-medium ${maintenance.urgent ? "text-amber-500" : "text-gray-700"}`}>{maintenance.label}</p>
            {item.lastMaintenanceDate && (
              <p className="text-xs text-gray-400 mt-0.5">Last: {fmtDate(item.lastMaintenanceDate, { month: "short", day: "numeric", year: "numeric" })}</p>
            )}
          </CardContent>
        </Card>
      </div>
      ...
      {item.maintenanceNotes && (
        <Card>
          ...
          <CardContent>
            <p className="text-sm text-gray-600 leading-relaxed">{item.maintenanceNotes}</p>
          </CardContent>
        </Card>
      )}
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-gray-400">{item.serviceHistory?.length ?? 0} records</span>
          </div>
        </CardHeader>
        <CardContent>
          {(!item.serviceHistory || item.serviceHistory.length === 0) ? (
            <div className="py-8 text-center text-gray-400 text-sm">No service records yet</div>
          ) : (
                      ...
                      <div className="flex-1 min-w-0 pb-4 border-b border-gray-50 last:border-0">
                        <div className="flex items-start justify-between gap-2 flex-wrap">
                          ...
                          {record.cost && Number(record.cost) > 0 && (
                            <span className="text-xs font-semibold text-gray-600">${Number(record.cost).toFixed(2)}</span>
                          )}
                        </div>
                        <p className="text-sm text-gray-700 mt-1.5 leading-relaxed">{record.description}</p>
