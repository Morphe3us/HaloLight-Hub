...
import { Plus, FileText, Trash2, ChevronRight, X, ChevronDown, ChevronUp } from "lucide-react";
...
type QuoteForm = {
  title: string; clientName: string; clientEmail: string; taxRate: string; notes: string; terms: string;
  leadId: string; clientPhone: string; clientCompany: string; clientAddress: string;
...
export default function Quotes() {
  ...
  const f = (field: keyof QuoteForm) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
                    ...
                    <tr key={q.id} className="hover:bg-muted/20 transition-colors group">
                      <td className="px-4 py-3">
                        ...
                        {(q as any).packageName && (
                          <p className="text-xs text-accent-foreground/70 truncate max-w-[160px]">{(q as any).packageName}</p>
                        )}
                      </td>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2 space-y-1.5">
                ...
                <Input value={form.title} onChange={f("title")} placeholder={t("quotes.title_placeholder")} />
              </div>
              <div className="space-y-1.5">
            ...
            >
              ...
              {(form.packageName || form.includedPrints || form.equipmentIds.length > 0) && (
                <span className="ml-auto text-xs text-primary font-medium">
                  {[form.packageName, form.includedPrints && `${form.includedPrints} prints`, form.equipmentIds.length > 0 && `${form.equipmentIds.length} equip.`].filter(Boolean).join(" · ")}
                </span>
              )}
            </button>
              ...
              <div className="border rounded-xl p-4 space-y-4 bg-muted/20">
                ...
                <div className="space-y-1.5">
                  ...
                  {equipmentItems.length === 0 ? (
                    <p className="text-xs text-muted-foreground">{t("quotes.equipment_empty")}</p>
                  ) : (
                ...
                {/* Pricing breakdown */}
                <div>
                  <Label className="mb-2 block">{t("quotes.pricing_breakdown_label")}</Label>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      <Label className="text-xs text-muted-foreground">{t("quotes.rental_price_label")}</Label>
                      <Input type="number" value={form.rentalPrice} onChange={f("rentalPrice")} placeholder="0" />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-xs text-muted-foreground">{t("quotes.options_price_label")}</Label>
                      <Input type="number" value={form.optionsPrice} onChange={f("optionsPrice")} placeholder="0" />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-xs text-muted-foreground">{t("quotes.delivery_fees_label")}</Label>
                      <Input type="number" value={form.deliveryFees} onChange={f("deliveryFees")} placeholder="0" />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-xs text-muted-foreground">{t("quotes.discount_amount_label")}</Label>
            ...
            <div className="space-y-1.5"><Label>{t("quotes.notes_section")}</Label><Textarea value={form.notes} onChange={f("notes")} rows={2} /></div>
            <div className="space-y-1.5"><Label>{t("quotes.terms_section")}</Label><Textarea value={form.terms} onChange={f("terms")} rows={2} placeholder={t("quotes.terms_placeholder")} /></div>
          </div>
          <DialogFooter>
            ...
            <Button onClick={handleCreate} disabled={createMutation.isPending || !form.clientName}>
              {createMutation.isPending ? t("quotes.creating") : t("quotes.create_quote_btn")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
