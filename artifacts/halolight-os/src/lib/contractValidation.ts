...
export interface OptionalIssue {
  field: string;
  placeholderKey: "placeholder_not_provided" | "placeholder_to_be_specified" | "placeholder_no_deposit";
}
...
export function validateContract(form: ContractFormData, provider: ProviderData): ValidationResult {
  ...
  if (!form.eventLocation.trim()) optional.push({ field: "field_event_location", placeholderKey: "placeholder_not_provided" });
  ...
  if (!form.paymentTerms.trim()) optional.push({ field: "field_payment_terms", placeholderKey: "placeholder_to_be_specified" });

  const unresolved = scanUnresolvedVars(form.content);

  return { blocking, optional, unresolved };
}

export function scanUnresolvedVars(content: string): string[] {
...
export function fillAllVariables(
  ...
  const today = new Date().toLocaleDateString(lang === "en" ? "en-GB" : lang, {
    year: "numeric",
    month: "long",
    day: "numeric",
  });
  ...
  const eventDate = form.eventDate
    ? new Date(form.eventDate).toLocaleDateString(lang === "en" ? "en-GB" : lang, {
        year: "numeric",
        month: "long",
        day: "numeric",
      })
  ...
  const totalFmt =
    form.value && Number(form.value) > 0
      ? `${form.currency || ""} ${Number(form.value).toFixed(2)}`.trim()
      : "";
  const depositFmt =
    form.depositAmount && Number(form.depositAmount) > 0
      ? `${form.currency || ""} ${Number(form.depositAmount).toFixed(2)}`.trim()
      : "";
  ...
  const replacements: Record<string, string> = {
    ...
    service_name: form.serviceName || placeholders.to_be_specified,
    ...
    total_amount: totalFmt || placeholders.to_be_specified,
    deposit_amount: depositFmt || placeholders.no_deposit,
