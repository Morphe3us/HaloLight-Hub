...
import {
  db, usersTable, invoices, quotes, customerSuccessScores,
} from "@workspace/db";
...
router.get("/admin/revenue", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  const allSentInvoices = await db.select({ total: invoices.total })
    .from(invoices).where(eq(invoices.status, "sent"));

  const allInvoices = await db.select({ total: invoices.total, status: invoices.status })
    .from(invoices);

  const totalRevenue = allPaidInvoices.reduce((s, r) => s + Number(r.total ?? 0), 0);
  const pipelineRevenue = allSentInvoices.reduce((s, r) => s + Number(r.total ?? 0), 0);
  ...
  const monthMap: Record<string, { month: string; label: string; revenue: number; invoiceCount: number }> = {};
  for (let i = 11; i >= 0; i--) {
  ...
  const lifetimeEstimate = Math.round(totalRevenue + avgMonthly * 12);
  ...
  }).from(quotes).groupBy(quotes.status);
  ...
  const totalQuotes = Object.values(quoteFunnelMap).reduce((s, v) => s + v.count, 0);
  ...
  const clientRevRows = await db.select({
    userId: invoices.userId,
  ...
  const leaderboard = await Promise.all(clientRevRows.map(async (row) => {
    ...
    }).from(customerSuccessScores).where(eq(customerSuccessScores.userId, row.userId));

    const totalRev = Math.round(Number(row.totalRevenue ?? 0));
    const avgBooking = row.invoiceCount > 0 ? Math.round(totalRev / row.invoiceCount) : 0;
  ...
  const tierRows = await db.select({
    ...
    .from(invoices)
    .innerJoin(customerSuccessScores, eq(invoices.userId, customerSuccessScores.userId))
    .where(eq(invoices.status, "paid"))
  ...
  const segments: Record<string, { label: string; revenue: number; count: number }> = {
  ...
  for (const row of segmentRows) {
    ...
    if (amt < 1000) { segments.micro!.revenue += amt; segments.micro!.count += 1; }
    else if (amt < 3000) { segments.small!.revenue += amt; segments.small!.count += 1; }
    else if (amt < 6000) { segments.medium!.revenue += amt; segments.medium!.count += 1; }
    else { segments.large!.revenue += amt; segments.large!.count += 1; }
  }
