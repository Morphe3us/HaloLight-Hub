...
import {
  db, aiConversations, aiMessages, aiSuggestedQuestions,
  supportTickets,
} from "@workspace/db";
...
router.post("/ai/conversations/:id/stream", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const history = await db.select()
      ...
      .where(eq(aiMessages.conversationId, id))
      .orderBy(asc(aiMessages.createdAt))
      .limit(14); // 12 + the one we just inserted
    ...
    const suggestedActions = buildSuggestedActions(content, sources);

    // ── 4. Build system prompt with injected context ─────────────────────────
    const systemPrompt = buildSystemPrompt(sources, user.fullName ?? undefined);
    ...
    for await (const chunk of provider.chat(chatMessages, systemPrompt, sources)) {
    ...
    if (conv.title === "New Conversation") {
      const newTitle = content.length > 52 ? content.slice(0, 49) + "…" : content;
      await db.update(aiConversations)
        .set({ title: newTitle, updatedAt: new Date(), providerName: provider.name })
    ...
    sendEvent({
      type: "done",
      message: { ...assistantMessage, sources: streamedSources, suggestedActions },
    });
  } catch (err) {
    req.log?.error(err, "AI stream error");
    sendEvent({ type: "error", error: "An unexpected error occurred. Please try again." });
  } finally {
    res.end();
  }
});
...
router.post("/ai/conversations/:id/messages", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  res.status(201).json({ userMessage, assistantMessage: { ...assistantMessage, sources: finalSources, suggestedActions } });
});
...
router.post("/ai/conversations/:id/escalate", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  const { subject, priority = "medium" } = req.body as { subject?: string; priority?: string };

  const ticketTitle = subject ?? `AI Chat: ${conv.title}`;
  ...
  const [ticket] = await db.insert(supportTickets).values({
    ...
    priority: (["low", "medium", "high", "urgent"].includes(priority) ? priority : "medium") as "low" | "medium" | "high" | "urgent",
    status: "open",
    category: "technical",
  }).returning();
...
// ─── Provider info ────────────────────────────────────────────────────────────
// GET /ai/provider
router.get("/ai/provider", requireAuth, async (req: Request, res: Response): Promise<void> => {
  const user = await getOrCreateUser(req);
  if (!user) { res.status(401).json({ error: "Unauthorized" }); return; }
