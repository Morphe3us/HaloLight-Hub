...
const DEFAULT_RULES = [
  {
    ...
    description: "Notifies clients who haven't completed onboarding within 3 days and creates a coaching task.",
    triggerType: "onboarding_stalled" as const,
    actionType: "in_app_notification" as const,
    triggerConfig: { stalledAfterDays: 3, completionThreshold: 50, cooldownHours: 48 },
    actionConfig: {
      notificationTitle: "Complete Your Onboarding",
      notificationBody: "You've completed {{completionPct}}% of your onboarding. Finish the remaining steps to unlock all HaloLight features.",
  ...
  },
  {
    ...
    description: "Sends re-engagement notification to users who haven't been active for 7+ days.",
  ...
  },
  {
    ...
    description: "Creates a coaching task when a client's academy progress falls below 25%.",
  ...
  },
  {
    ...
    description: "Notifies clients who haven't created any events 7 days after signing up.",
  ...
  },
  {
    ...
    description: "Opens a support follow-up ticket when a client hasn't sent any quotes 7 days in.",
  ...
  },
  {
    ...
    description: "Triggers a reorder recommendation when consumable stock falls below 14 days remaining.",
  ...
  },
  {
    ...
    description: "Sends a notification and creates a support ticket when equipment warranty expires within 30 days.",
    ...
    actionConfig: {
      ...
      notificationBody: "The warranty on {{productModel}} expires in {{daysUntilExpiry}} days. Review your coverage options.",
  ...
  },
  {
    ...
    description: "Recognises top performers and creates a growth coaching task.",
    triggerType: "high_performer_detected" as const,
    actionType: "coaching_task_creation" as const,
    triggerConfig: { minQuotes: 5, minEvents: 3, periodDays: 30, cooldownHours: 168 },
    actionConfig: {
  ...
  },
  {
    ...
    description: "Identifies clients with strong upsell signals and creates a revenue recommendation.",
  ...
  },
  {
    ...
    description: "Generates coaching recommendations for clients who completed onboarding but have low academy progress.",
    triggerType: "coaching_recommendation_generated" as const,
    actionType: "coaching_task_creation" as const,
    triggerConfig: { progressThreshold: 50, completionThreshold: 60, cooldownHours: 168 },
    actionConfig: {
...
async function seed() {
  ...
  if (existing.length >= DEFAULT_RULES.length) {
    console.log(`Automation rules already seeded (${existing.length} rules found)`);
    return;
  }
