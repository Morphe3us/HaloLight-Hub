import {
  ...
  userLessonProgress, lessons, events, quotes, quoteItems, invoices, invoiceItems,
  supportTickets, supportTicketReplies, communityChannels, communityPosts, communityReplies,
} from "@workspace/db";
...
export async function seedPhase5() {
  ...
  const allLessons = await db.select({ id: lessons.id }).from(lessons).limit(10);

  for (const demo of DEMO_CLIENTS) {
    const existing = await db.select().from(usersTable).where(eq(usersTable.clerkId, demo.clerkId));
    let userId: string;

    const lastActive = new Date(Date.now() - demo.daysAgo * 24 * 60 * 60 * 1000);
    ...
    await db.update(usersTable).set({ updatedAt: lastActive }).where(eq(usersTable.id, userId));

    // Onboarding progress
    const stepsToComplete = allSteps.slice(0, demo.onboardingSteps);
    for (const step of stepsToComplete) {
      const exists = await db.select().from(userOnboardingProgressTable)
      ...
      if (!stepExists) {
        await db.insert(userOnboardingProgressTable).values({
          userId,
          stepId: step.id,
          completedAt: new Date(lastActive.getTime() + 24 * 60 * 60 * 1000),
        });
      }
    }
    ...
    for (let i = 0; i < Math.min(demo.academyLessons, allLessons.length); i++) {
      ...
      const exists = await db.select().from(userLessonProgress)
        .where(eq(userLessonProgress.userId, userId));
      if (!exists.some((e) => e.lessonId === lesson.id)) {
        await db.insert(userLessonProgress).values({
          userId,
          lessonId: lesson.id,
          completedAt: new Date(lastActive.getTime() + 2 * 24 * 60 * 60 * 1000),
        });
      }
    }
    ...
    for (let i = 0; i < eventsToCreate; i++) {
      const futureDate = new Date(lastActive.getTime() + (i + 7) * 24 * 60 * 60 * 1000);
      await db.insert(events).values({
        userId,
        title: [`Birthday Party @ Grand Ballroom`, `Corporate Gala — ${demo.companyName}`, `Wedding Reception — Ritz Hotel`][i] ?? `Event ${i + 1}`,
        eventDate: futureDate,
        location: [`The Grand Ballroom, NYC`, `Hilton Conference Center`, `Ritz-Carlton Hotel`][i] ?? "Venue TBD",
        status: (["upcoming", "active", "completed"] as const)[i] ?? "upcoming",
        expectedGuests: [120, 250, 80][i] ?? 100,
        notes: "Client demo event",
      });
    }

    // Quotes
    const existingQuotes = await db.select().from(quotes).where(eq(quotes.userId, userId));
    ...
    for (let i = 0; i < quotesToCreate; i++) {
      ...
      const [q] = await db.insert(quotes).values({
        userId,
        quoteNumber: `Q-${userId.slice(0, 4).toUpperCase()}-${String(i + 1).padStart(3, "0")}`,
        title: [`Open Air Booth Package`, `Enclosed Booth Premium`, `LED Ring Light Add-on`, `Full Day Package`][i] ?? `Quote ${i + 1}`,
    ...
    // Invoices
    const existingInvoices = await db.select().from(invoices).where(eq(invoices.userId, userId));
    ...
    for (let i = 0; i < invoicesToCreate; i++) {
      ...
      const [inv] = await db.insert(invoices).values({
        userId,
        invoiceNumber: `INV-${userId.slice(0, 4).toUpperCase()}-${String(i + 1).padStart(3, "0")}`,
        title: [`Photobooth Rental — Q1 Event`, `Premium Package — Corporate Gala`, `Add-on Services`][i] ?? `Invoice ${i + 1}`,
        clientName: demo.fullName ?? demo.email,
        clientEmail: demo.email,
        subtotal: String(amount),
        taxRate: "8.5",
        ...
        dueDate: new Date(Date.now() + (isPaid ? -7 : 14) * 24 * 60 * 60 * 1000),
    ...
    // Community posts
    const existingPosts = await db.select().from(communityPosts).where(eq(communityPosts.userId, userId));
    if (existingPosts.length < demo.communityPosts) {
      const channels = await db.select({ id: communityChannels.id }).from(communityChannels).limit(1);
      ...
      if (firstChannelRow) {
        ...
        for (let i = existingPosts.length; i < demo.communityPosts; i++) {
          await db.insert(communityPosts).values({
            channelId,
            userId,
            title: [`My first photobooth event went amazing!`, `Tips for managing large events`, `How I scaled to 5 bookings/month`][i] ?? `Post ${i + 1}`,
    ...
    // Support tickets
    const existingTickets = await db.select().from(supportTickets).where(eq(supportTickets.userId, userId));
    if (existingTickets.length < demo.tickets) {
      for (let i = existingTickets.length; i < demo.tickets; i++) {
        const [ticket] = await db.insert(supportTickets).values({
          userId,
          ticketNumber: `TK-${userId.slice(0, 4).toUpperCase()}-${String(Date.now()).slice(-4)}`,
          title: [`Hardware question about print speed`, `How to create a custom template`][i] ?? `Support request ${i + 1}`,
          description: "I have a question about my equipment setup and need some guidance.",
